...
c                                 molar composition
            ! *                      (pcomp(l,i), l = 1, icomp)
				...
				! ! This is in case you want to append the output at
				! ! the end of the file.
 				! ! Open the final ouput. Get to the end of file.
				! open(0003, file='xoutput1.xyz')
				! nlin = 0
				! do while(.true.)
         		! read(0003,*,iostat=ios)
					! if( ios < 0 ) then ! end of file is reached
            		! exit
					! else
						! nlin = nlin + 1
					! end if
            ...
            ! *                      (pcomp(l,i), l = 1, icomp)
				...
				! This is in case you just want to write.
            write (555,1030) pname(i),
     *                      props(17,i)*props(16,i)/psys(17)*1d2,
     *                      props(1,i)*props(16,i)/psys(1)*1d2,
     *                      props(16,i)/psys(16)*1d2,
     *                      props(16,i),
